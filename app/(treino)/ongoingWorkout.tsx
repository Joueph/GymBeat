...
export default function OngoingWorkoutScreen() {
  ...
  const { user } = useAuth();
  // We'll get the treinoId, fichaId, and optionally logId from the navigation parameters
  ...
  // Novo estado para controlar qual exercício do bi-set está visível
  const [biSetToggle, setBiSetToggle] = useState(false);

  // Novo estado para o modo de edição de bi-set
  const [isBiSetEditing, setIsBiSetEditing] = useState(false);

  // Novo estado para saber qual exercício está sendo editado no modal
  ...
  // Lógica para determinar o exercício atual a ser exibido (considerando o bi-set)
  ...
  useEffect(() => {
    const startTime = new Date(); // Usado apenas se um novo log for criado
    const fetchTreino = async () => {
      ...
      try {
        ...
        if (logId && typeof logId === 'string') {
            // Se um logId foi passado, estamos continuando um treino.
            ...
            if (logSnap.exists()) {
                ...
                // Calcula o índice do exercício atual com base nos exercícios já feitos
                const exerciciosFeitos = logData.exerciciosFeitos || [];
                const proximoIndice = exerciciosFeitos.length;
                
                if (proximoIndice < treinoData.exercicios.length) {
                    setCurrentExerciseIndex(proximoIndice);
                }
            } else {
                throw new Error("Log de treino para continuar não encontrado.");
            }
        } else if (user) {
          // Se não há logId, criamos um novo log para um novo treino.
          setHorarioInicio(startTime); // Define o horário de início para o novo log
          ...
          setActiveLogId(newLogId); // Armazena o ID do novo log
        }
    ...
    if (newCompletedSets >= currentExercise.series.length) {      
        if (activeLogId) {
            try {
                ...
                const logSnap = await getDoc(logRef);
                const logData = logSnap.data();
                const currentExerciciosFeitos = logData?.exerciciosFeitos || [];
                const exercisesToAdd = isBiSet
                    ? [treino.exercicios[currentExerciseIndex], treino.exercicios[currentExerciseIndex + 1]]
                    : [treino.exercicios[currentExerciseIndex]];
                const newExerciciosFeitos = [...currentExerciciosFeitos, ...exercisesToAdd];
                await updateDoc(logRef, {
                    exercicios: treino.exercicios, 
                    exerciciosFeitos: newExerciciosFeitos,
                });
      ...
      if (nextIndex < treino.exercicios.length) {
        ...
        try {
          if (activeLogId) {
            await addLog({ horarioFim, exerciciosFeitos: treino.exercicios, exercicios: treino.exercicios }, activeLogId);
  ...
  const handleSaveExerciseChanges = () => {
    ...
    if (isLeaderOfBiSet) {
      ...
      const newPartnerSeries = updatedExercise.series.map((leaderSerie, index) => {
        ...
        return {
          ...leaderSerie,
          repeticoes: oldPartnerSet?.repeticoes || leaderSerie.repeticoes,
          peso: oldPartnerSet?.peso ?? leaderSerie.peso,
        };
      });
  ...
  const handleBack = () => {
    const exitWorkout = async () => {
      // Em vez de apagar, vamos marcar o log como cancelado.
      // Isso evita problemas de permissão e mantém um registro, se necessário.
      if (activeLogId) {
        try {
          const { getFirestore, doc, updateDoc } = require('firebase/firestore');
          const db = getFirestore();
          await updateDoc(doc(db, 'logs', activeLogId), {
            status: 'cancelado' // Adiciona um campo para indicar o cancelamento
          });
          router.back(); // Volta para a tela anterior após o sucesso
        } catch (error) {
          ...
          router.back(); // Permite que o usuário saia mesmo se a atualização falhar
        }
      }
    };
    Alert.alert(
        ...
        <View style={styles.progressListItem}>
            ...
            <View style={styles.exerciseContent}>
                ...
                <Text style={styles.modalExerciseDetails}>
                  ...
                  {item.series.filter(s => s.type === 'dropset').length > 0 && ` + ${item.series.filter(s => s.type === 'dropset').length} dropsets`}
                </Text>
            </View>
        </View>
    );
    ...
    <GestureHandlerRootView style={{ flex: 1 }}>
      <SafeAreaView style={styles.safeArea}>
        ...
        <View style={styles.content}>
          <TouchableOpacity style={styles.timerContainer} onPress={handleSkipRest} disabled={!isResting}>
            <View>
              ...
              <Text style={styles.timerText}>{isResting ? formatTime(restTime) : formatTime(maxRestTime)}</Text>
            </View>
          </TouchableOpacity>
          ...
          <View style={styles.bottomSectionContainer}>
              ...
              {isBiSet && biSetPartnerExercise ? (
                ...
                <>
                  ...
                  <View style={styles.detailsContainer}>
                      <View style={styles.detailItem}><FontAwesome name="clone" size={20} color="#ccc" /><Text style={styles.detailValue}>{completedNormalSeriesCount}/{totalNormalSeries}</Text><Text style={styles.detailLabel}>Séries</Text></View>
                      {currentSet && <NormalSetDetails currentSet={currentSet} />}
                  </View>
            ...
            <View style={styles.actionsContainer}>
              <TouchableOpacity style={styles.actionButton} onPress={handleEdit}>
                <FontAwesome name={isBiSetEditing ? "check" : "pencil"} size={24} color={isBiSetEditing ? "#1cb0f6" : "#fff"} />
                <Text style={[styles.actionButtonText, isBiSetEditing && { color: '#1cb0f6' }]}>Editar</Text>
              </TouchableOpacity>
        ...
        <Modal visible={isExerciseListVisible} animationType="slide" presentationStyle="pageSheet" onRequestClose={() => setExerciseListVisible(false)}>
          <SafeAreaView style={styles.modalSafeArea}>
            <View style={styles.modalHeader}>
              ...
              <TouchableOpacity onPress={() => setExerciseListVisible(false)}><FontAwesome name="close" size={24} color="#fff" /></TouchableOpacity>
            </View>
            <FlatList data={treino.exercicios} keyExtractor={(item, index) => `exercicio-lista-${index}`} renderItem={renderExerciseProgressItem} contentContainerStyle={{ paddingHorizontal: 20, paddingTop: 20 }} />
          </SafeAreaView>
        </Modal>
        ...
        <Modal animationType="slide" presentationStyle="pageSheet" visible={isEditExerciseModalVisible} onRequestClose={() => setEditExerciseModalVisible(false)}>
          <SafeAreaView style={styles.modalSafeArea}>
            <View style={styles.modalHeader}><Text style={styles.modalTitle}>Editar Exercício</Text><TouchableOpacity onPress={() => setEditExerciseModalVisible(false)}><FontAwesome name="close" size={24} color="#fff" /></TouchableOpacity></View>
            {exercicioSendoEditado && ( <View style={styles.editingExercisePreview}>{exercicioSendoEditado.modelo.imagemUrl && ( <VideoListItem uri={exercicioSendoEditado.modelo.imagemUrl} style={styles.editingExerciseVideo} /> )}<View style={{ flex: 1 }}><Text style={styles.editingExerciseName}>{exercicioSendoEditado.modelo.nome}</Text><Text style={styles.editingExerciseMuscleGroup}>{exercicioSendoEditado.modelo.grupoMuscular}</Text></View></View> )}
            <DraggableFlatList data={editingSeries} style={{ width: '100%' }} contentContainerStyle={styles.modalScrollViewContent} keyExtractor={(item) => item.id!} onDragEnd={({ data }) => { if (!exercicioSendoEditado?.isBiSet) setEditingSeries(data); }}
        ...
        <Modal visible={isExerciseDetailModalVisible} animationType="slide" presentationStyle="pageSheet" onRequestClose={() => setExerciseDetailModalVisible(false)}>
          <SafeAreaView style={styles.modalSafeArea}>
            <View style={styles.modalHeader}><Text style={styles.modalTitle}>Detalhes do Exercício</Text><TouchableOpacity onPress={() => setExerciseDetailModalVisible(false)}><FontAwesome name="close" size={24} color="#fff" /></TouchableOpacity></View>
            <View style={styles.detailModalContentWrapper}><ScrollView>
                <View>{currentExercise?.modelo.imagemUrl && <VideoListItem uri={currentExercise.modelo.imagemUrl} style={styles.detailModalVideo} />}<Text style={styles.detailModalExerciseName}>{currentExercise?.modelo.nome}</Text></View>
